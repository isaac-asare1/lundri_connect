...
const ARKESEL_OTP_BASE_URL = "https://sms.arkesel.com/api/otp";
const ARKESEL_SENDER_ID = "Lundri";
...
exports.sendPhoneOtp = onCall(
  ...
  async (request) => {
    const phoneNumber = normalizeGhanaPhoneNumber(
    ...
    const rateLimitRef = otpRateLimitRef(phoneNumber);
    await reserveOtpSend(rateLimitRef);

    let response;
    let payload;

    try {
      response = await fetch(`${ARKESEL_OTP_BASE_URL}/generate`, {
        method: "POST",
        headers: {
          "api-key": ARKESEL_API_KEY.value(),
        ...
        body: JSON.stringify({
          ...
          medium: "sms",
          message:
            "Your Lundri verification code is %otp_code%. It expires in %expiry% minutes.",
          number: phoneNumber,
      ...
      logger.error("Arkesel OTP generate request failed.", {
    ...
    if (!response.ok || payload?.code !== "1000") {
      logger.warn("Arkesel rejected OTP generate request.", {
        phoneHash: hashPhoneNumber(phoneNumber),
      ...
      throw mapArkeselSendError(payload);
    }
...
exports.verifyPhoneOtp = onCall(
  ...
  async (request) => {
    ...
    const rateLimitRef = otpRateLimitRef(phoneNumber);
    await assertOtpVerifyAllowed(rateLimitRef);

    let response;
    let payload;

    try {
      response = await fetch(`${ARKESEL_OTP_BASE_URL}/verify`, {
        method: "POST",
        headers: {
          "api-key": ARKESEL_API_KEY.value(),
          "Content-Type": "application/json",
      ...
      logger.error("Arkesel OTP verify request failed.", {
        phoneHash: hashPhoneNumber(phoneNumber),
        error: error?.message || String(error),
      });
    ...
    if (response.ok && payload?.code === "1100") {
      await clearOtpRateLimit(rateLimitRef);
    ...
    if (payload?.code === "1104") {
      await recordFailedOtpAttempt(rateLimitRef);
    ...
    logger.warn("Arkesel rejected OTP verification.", {
      phoneHash: hashPhoneNumber(phoneNumber),
      httpStatus: response.status,
      providerCode: payload?.code || null,
      providerMessage: payload?.message || null,
    ...
    throw new HttpsError(
      "failed-precondition",
      "The verification code could not be verified. Request a new code and try again.",
    );
  },
);

function normalizeGhanaPhoneNumber(value) {
...
function otpRateLimitRef(phoneNumber) {
  const phoneHash = hashPhoneNumber(phoneNumber);
  return db.collection("otp_rate_limits").doc(phoneHash);
}

async function reserveOtpSend(ref) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    let effectiveWindowStartedAt = windowStartedAt;

    const windowExpired =
      !windowStartedAt ||
      typeof windowStartedAt.toMillis !== "function" ||
      now.toMillis() - windowStartedAt.toMillis() >= windowMs;

    if (windowExpired) {
      sendCount = 0;
      effectiveWindowStartedAt = now;
    }

    if (
      lastSentAt &&
      typeof lastSentAt.toMillis === "function" &&
      now.toMillis() - lastSentAt.toMillis() < cooldownMs
    ) {
    ...
    tx.set(
      ref,
      {
        windowStartedAt: effectiveWindowStartedAt || now,
        lastSentAt: now,
...
async function assertOtpVerifyAllowed(ref) {
  ...
  const data = snap.data() || {};
  const verifyAttempts = Number(data.verifyAttempts || 0);
...
async function recordFailedOtpAttempt(ref) {
  await ref.set(
    {
...
async function clearOtpRateLimit(ref) {
  try {
    await ref.delete();
  } catch (error) {
    logger.warn("Unable to clear OTP rate-limit document.", {
      error: error?.message || String(error),
    });
  }
}
...
function mapArkeselSendError(payload) {
  ...
  if (
    ...
    return new HttpsError(
      "resource-exhausted",
      "Verification SMS is temporarily unavailable. Please try again later.",
    );
  }

  if (
    message.includes("sender") ||
    message.includes("sender id")
  ) {
    return new HttpsError(
      "failed-precondition",
      "The verification service is not fully configured yet.",
    );
  }
